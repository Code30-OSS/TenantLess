//! Build script — the `/ui` embedded-SPA freshness guard (WEBUI-03, RESEARCH Pitfall 4).
//!
...
//!
//!   * **Re-embed on change (Pitfall 4a):** emit `cargo:rerun-if-changed=../frontend/dist`
//!     so cargo re-runs — and thus re-invokes the `include_dir!` embed — whenever the built
//!     SPA changes. Without this, `cargo build` after `npm run build` ships the old bytes.
//!   * **Fail loudly on a missing dist (Pitfall 4c):** hard-`panic!` (a build error) if
    ...
    if !index.exists() {
        panic!(
            "frontend/dist/index.html is missing — the /ui embed (src/ui.rs `include_dir!`) \
             ...
             `cd frontend && npm ci && npm run build` (WEBUI-03, RESEARCH Pitfall 4)."
        );
    }
}
