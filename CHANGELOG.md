...
- **Server-wide database timeouts** — `DB_STATEMENT_TIMEOUT_MS` (default 10000) sets a
  ...
  returns `504`; pool exhaustion (the acquire timeout elapsing) returns `503 ServiceUnavailable`
  with `Retry-After: 1`. The cost query keeps its own tighter app-level deadline (still a `400`
  "query too expensive") layered on top, and now inherits the server-wide `statement_timeout`
  rather than forcing its own — so a tighter configured global is always respected. The startup
  schema preflight is exempt from the runtime timeout, so a first-run migration over a large
  estate cannot be cancelled mid-provision.
- **`$filter` size cap** — a `$filter` over 2 KiB or 200 tokens is rejected with the fixed
  ...
  parser's recursion depth. Applied to both the resource-list and the roleAssignments
  `$filter` surfaces.
...
- **Cost query body-size cap** — the Cost Management query body is bounded at 64 KiB, enforced
  ...
  body returns `413 RequestEntityTooLarge`. A non-JSON `Content-Type` is rejected `415`
  (preserving the JSON media-type contract), and a size-budget rejection is kept distinct from
